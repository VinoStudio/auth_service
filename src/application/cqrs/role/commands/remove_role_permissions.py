...
class RemoveRolePermissionsCommand(BaseCommand):
    role_name: str
...
@dataclass(frozen=True)
class RemoveRolePermissionsCommandHandler(
    CommandHandler[RemoveRolePermissionsCommand, domain.Role]
):
    ...
    async def handle(
        ...
        for permission_name in command.new_permissions:
            ...
            role.remove_permission(db_permission)
